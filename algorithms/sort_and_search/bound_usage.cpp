...
{ 
    ...
    // find the first number which is greater than 23     
    auto item = upper_bound(nums.begin(), nums.end(), 23);
    cout << "item greater than 23 is:" << *item << endl; 
    
    // find the first number which is equal or greater to 24
    item = lower_bound(nums.begin(), nums.end(), 24); 
    cout << "lower_bound for 24 is:" << *item << endl;

    item = lower_bound(nums.begin(), nums.end(), 23); 
    cout << "lower_bound for 23 is:" << *item << endl;
} 
